...
const getResult = (entries: number[]) => {
    ...
    for (let index1 = 0; index1 < entries.length && correctEntries.length == 0; index1++) {
        for (
            ...
            const index3 = entries.findIndex(
                (n) => n === 2020 - (entries[index1] + entries[index2]) && n != index1 && n != index2
            );
            if (index3 > -1) correctEntries.push(entries[index1], entries[index2], entries[index3]);
        }
    }
